...
/obj/item/gun/energy/laser/makeshiftlasrifle/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/stock_parts/cell/computer/super) && !upgraded_cell) //We want to upgrade are cell
		var/obj/item/stock_parts/cell/computer/super = B
			qdel(B)
			upgraded_cell = TRUE
			maxcellcharge = 2500
		else
			...
			return
	if(istype(I, /obj/item/assembly/timer) && !upgraded_firerate) //We want to upgrade are fire rate
		var//obj/item/assembly/timer = T
			qdel(T)
			upgraded_firerate = TRUE
			fire_delay = 10
		else
			to_chat(user, "<span class='notice'>There's already a second cell timer in [src]!</span>")
			return
	if(istype(I,  /obj/item/pipe) && !upgraded_rifleing) //We want to lower the spread
		var//obj/item/assembly/timer = T
			qdel(T)
